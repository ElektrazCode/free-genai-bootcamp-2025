 ...
 `[1:], _filepath.Base(os.Args[0]))
	...
	if args.Help {
		...
		switch _strings.ToLower(args.Args[0]) {
			case "init":
				_fmt.Println("Initialize the database")
				_fmt.Println()
				...
				return
			case "migrate":
				_fmt.Println("Migrate the database")
				_fmt.Println()
				...
				return
			case "seed":
				_fmt.Println("Seed the database")
				_fmt.Println()
		...
		switch _strings.ToLower(target) {
			...
			case "init":
				...
				if expected > len(args.Args) {
					...
					logger.Printf("not enough arguments for target \"Init\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "Init")
				}
			...
			case "migrate":
				...
				if expected > len(args.Args) {
					// note that expected and args at this point include the arg for the target itself
					// so we subtract 1 here to show the number of args without the target.
					logger.Printf("not enough arguments for target \"Migrate\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "Migrate")
				}
			...
			case "seed":
				...
				if expected > len(args.Args) {
					// note that expected and args at this point include the arg for the target itself
					// so we subtract 1 here to show the number of args without the target.
					logger.Printf("not enough arguments for target \"Seed\", expected %v, got %v\n", expected-1, len(args.Args)-1)
					os.Exit(2)
				}
				if args.Verbose {
					logger.Println("Running target:", "Seed")
				}
